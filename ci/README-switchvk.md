...
For a pinned deployment, copy the selected Release archive SHA-256 into
`ci/switchvk.lock`, or set the non-secret repository variable
...
If a sibling `switchVK/nvk-switch-X.Y.Z` or
`switch-nvk/nvk-switch-X.Y.Z` package exists, simply run:
...
```sh
SWITCH_NVK_ROOT=/path/to/nvk-switch-X.Y.Z bash build_local.sh
```
